...
async function searchPlaces() {
  ...
  do {
    ...
    page += 1
    console.log(
      `  página ${page}: ${data.places?.length ?? 0} resultado(s) (acum ${results.length})`,
    )
...
async function main() {
  if (args['from-file']) {
    ...
    if (!push) {
      console.log(
        'Nada que hacer sin --push cuando usas --from-file (los datos ya están en el JSON).',
      )
      return
    }
  ...
  if (push) {
    ...
    const outFile =
      args['out-file'] ?? path.join(outDir, `${args.query.toLowerCase().replace(/\s+/g, '-')}.json`)
    await writeFile(
      outFile,
