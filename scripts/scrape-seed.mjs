...
for (const key of ['NEXT_PUBLIC_SUPABASE_URL', 'SUPABASE_SERVICE_ROLE_KEY']) {
  if (!process.env[key]) {
    console.error(
      `Falta ${key} en env. Corre con: node --env-file=.env.local scripts/scrape-seed.mjs`,
    )
    process.exit(1)
  }
}
...
async function main() {
  ...
  const totalRuns = resolved.reduce((acc, j) => acc + j.queries.length * locations.length, 0)
  console.log(
