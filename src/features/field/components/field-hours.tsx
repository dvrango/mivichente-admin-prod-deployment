...
function sameDays(hours: WeeklyHours, days: number[]): boolean {
  ...
  if (!days.every((d) => hours[d])) return false
  const first = hours[days[0]]!
  return days.every(
    (d) => hours[d]!.opens_at === first.opens_at && hours[d]!.closes_at === first.closes_at,
  )
}
...
export function FieldHours({
  ...
  const firstEntry = Object.values(value).find(Boolean)
  ...
  function changeDay(day: number, field: 'opens_at' | 'closes_at', time: string) {
    const existing = value[day]
    if (!existing) return
    onHoursChange({ ...value, [day]: { ...existing, [field]: time } })
  }
      ...
      {mode === 'custom' && (
        <ul className="divide-y rounded-xl border">
          {EVERY_DAY.map((day) => {
            ...
            return (
              <li key={day} className="flex items-center gap-3 px-3 py-2">
                <button
                  type="button"
                  onClick={() => toggleDay(day, !hours)}
                  aria-pressed={!!hours}
                  className={`min-h-11 w-14 shrink-0 rounded-lg text-sm font-medium ${
                    hours ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'
                  }`}
                >
                  {DAY_LABELS[day]}
                </button>
                {hours ? (
                  <div className="flex flex-1 items-center gap-2">
                    <input
                      type="time"
                      aria-label={`Abre ${DAY_LABELS[day]}`}
                      value={hours.opens_at}
                      onChange={(e) => changeDay(day, 'opens_at', e.target.value)}
                      className="border-input h-12 min-w-0 flex-1 rounded-lg border px-2 text-base"
                    />
                    <span className="text-muted-foreground">–</span>
                    <input
                      type="time"
                      aria-label={`Cierra ${DAY_LABELS[day]}`}
                      value={hours.closes_at}
                      onChange={(e) => changeDay(day, 'closes_at', e.target.value)}
                      className="border-input h-12 min-w-0 flex-1 rounded-lg border px-2 text-base"
                    />
                  </div>
                ) : (
